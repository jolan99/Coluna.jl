...
function new_root(sp::TreeSearchSpace, input)
    ...
    return Node(
        tree_order, false, 0, nothing, nodestate, "", store_records!(sp.reformulation), false
    )
end
...
function after_conquer!(space::TreeSearchSpace, output)
...
# Conquer
function get_input(::AbstractConquerAlgorithm, space::TreeSearchSpace, node::Node)
    return ConquerInput(node, space.conquer_units_to_restore)
end
